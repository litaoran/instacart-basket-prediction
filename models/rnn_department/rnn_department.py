...
class rnn(TFBaseModel):
    ...
    def get_input_sequences(self):
        ...
        self.num_products_from_department_history = tf.placeholder(tf.int32, [None, 100])
        self.next_is_ordered = tf.placeholder(tf.int32, [None, 100])

        self.keep_prob = tf.placeholder(tf.float32)
        self.is_training = tf.placeholder(tf.bool)

        x_department = tf.one_hot(self.department_id, 30)
        ...
        num_products_from_department_history = tf.one_hot(self.num_products_from_department_history, 50)
        ...
        num_products_from_department_history_scalar = tf.expand_dims(tf.cast(self.num_products_from_department_history, tf.float32) / 50.0, 2)
        ...
        x = tf.concat([x_history, x_department, x_user], axis=2)
